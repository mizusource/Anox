...
from AnonX.utils.decorators.play import PlayWrapper, PlayyWrapper
...
from AnonX.utils.loggerr import play_logss
...
async def play_commnd(
    ...
    if audio_telegram:
        ...
        if await YouTube.exists(url):
            ...
            return await play_logs(
                message, client, streamtype="M3u8 or Index Link"
            )
    else:
    ...
    if str(playmode) == "Direct":
        ...
        try:
            ...
            return await mystic.edit_text(err)
        await mystic.delete()
        return await play_logs(message, client, streamtype=streamtype)
    else:
        if plist_type:
            ran_hash = "".join(
                random.choices(
            ...
            return await play_logs(
                message, client, streamtype=f"Playlist : {plist_type}"
            )
        else:
            if slider:
                ...
                return await play_logs(
                    message, client, streamtype=f"Searched on Youtube"
                )
            else:
                ...
                return await play_logs(
                    message, client, streamtype=f"URL Searched Inline"
                )
...
async def play_commnd(
    ...
    if str(playmode) == "Direct":
        ...
        try:
            ...
            return await mystic.edit_text(err)
        await mystic.delete()
        return await play_logss(message, client, streamtype=streamtype)
    else:
        if plist_type:
            ran_hash = "".join(
                random.choices(
