...
async def stats_global(client, message: Message, _):
    ...
    await message.reply_photo(
        ...
        caption=_["gstats_11"].format(app.name),
        reply_markup=upl,
    )
...
async def gstats_global(client, message: Message, _):
    ...
    title = title.title()
    final = f"Top Most Played Track on {app.name}\n\n**Title:** {title}\n\nPlayed** {co} **times"
...
async def top_users_ten(client, CallbackQuery: CallbackQuery, _):
    ...
    if what in ["Users", "Chats"]:
        ...
        temp = (
            _["gstats_5"].format(limit, app.name)
            if what == "Chats"
            else _["gstats_6"].format(limit, app.name)
        )
        msg = temp + msg
...
async def back_buttons(client, CallbackQuery, _):
    ...
    if command == "GlobalStats":
        ...
        med = InputMediaPhoto(
            media=config.GLOBAL_IMG_URL,
            caption=_["gstats_10"].format(app.name),
        )
        try:
            await CallbackQuery.edit_message_media(
                media=med, reply_markup=upl
