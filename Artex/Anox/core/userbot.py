...
class Userbot(Client):
    ...
    async def start(self):
        ...
        if config.STRING1:
            ...
            try:
                await self.one.join_chat("AR_T_EX")
                await self.one.join_chat("SUBBORT_ARTEX")
                await self.one.join_chat("X1_ID")
                await self.one.join_chat("CAN_SOURCE")
                await self.one.join_chat("Qrhel_Bot")
            except:
                pass
            assistants.append(1)
            ...
            try:
                await self.one.send_message(
                    config.LOG_GROUP_ID, f"**» ᴀssɪsᴛᴀɴᴛ ᴏɴᴇ sᴛᴀʀᴛᴇᴅ :**\n\n✨ ɪᴅ : `{self.one.id}`\n❄ ɴᴀᴍᴇ : {self.one.name}\n💫 ᴜsᴇʀɴᴀᴍᴇ : @{self.one.username}"
                )
            except:
        ...
        if config.STRING2:
            ...
            try:
                await self.two.send_message(
                    config.LOG_GROUP_ID, f"• الحساب المساعد بدا بالعمل ✅"
                )
            except:
                LOGGER(__name__).error(
